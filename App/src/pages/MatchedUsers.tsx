...
const MatchedUsersPage = () => {
	...
	const handleRefresh = async () => {
		...
		try {
			const message = JSON.parse(matchMessage || "{}");
	...
	return (
		<div className="p-4 max-w-md mx-auto relative">
			...
			{loading ? (
				...
				matchedUsers.map((match: MatchedUser, index: number) => (
					<div
						key={index}
					...
					>
						...
						<p>
							<b>Trip Duration:</b>{" "}
							{typeof match.tripDuration === "number"
								? `~${Math.floor(match.tripDuration / 60)} minutes`
								: "N/A"}
						</p>
						<p>
							<b>Trip Cost:</b> ~{Math.floor(match.tripCost ?? 0)} - {Math.floor((match.tripCost ?? 0) + (0.3 * (match.tripCost ?? 0)))}{" "}
							/-
						</p>
